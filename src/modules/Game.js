...
class Game {
  ...
  set wave(val) {
    ...
    if (this.stage && this.stage.hud) {
      ...
      if (!isNaN(val) && val > 0) {
        this.stage.hud.waveStatus = 'مرحله ' + val + ' از ' + this.level.waves;
      } else {
  ...
  addFullscreenLink() {
    ...
    this.stage.hud.fullscreenLink = 'تمام صفحه (f)';
  }
  addMuteLink() {
    ...
    this.stage.hud.muteLink = 'بی صدا (m)';
  }
  ...
  addPauseLink() {
    ...
    this.stage.hud.pauseLink = 'توقف (p)';
  }
  ...
  addLinkToLevelCreator() {
    ...
    this.stage.hud.levelCreatorLink = 'سازنده سطح (c)';
  }
  ...
  bindEvents() {
    ...
    document.addEventListener('fullscreenchange', () => {
      if (document.fullscreenElement) {
        this.stage.hud.fullscreenLink = 'خارج از حالت تمام صفحه (f)';
      } else {
        this.stage.hud.fullscreenLink = 'تمام صفحه (f)';
      }
    });
  ...
  pause() {
    this.stage.hud.pauseLink = this.paused ? 'توقف (p)' : 'شروع مجدد (p)';
  ...
  mute() {
    this.stage.hud.muteLink = this.muted ? 'بی صدا (m)' : 'صدا (m)';
  ...
  win() {
    ...
    // Set content
    message.textContent = `تبریک شما برنده شدید اینم از کد تخفیف`;
    code.textContent = window.uniqueCode;
    copyButton.textContent = 'کپی کد';

    // Add click handler for copy button
    copyButton.addEventListener('click', () => {
      navigator.clipboard.writeText(window.uniqueCode)
        .then(() => {
  ...
  getScoreMessage() {
    ...
    if (percentage === 100) {
      scoreMessage = 'برد بی عیب و نقص';
    }

    if (percentage < 100) {
      scoreMessage = 'خیلی نزدیک بود!!';
    }

    if (percentage <= 95) {
      scoreMessage = 'واقعا امتیاز قابل توجهی';
    }

    if (percentage <= 85) {
      scoreMessage = 'امتیاز خوبی بود!';
    }

    if (percentage <= 75) {
      scoreMessage = 'خیلی خوب بود!';
    }

    if (percentage <= 63) {
      scoreMessage = 'بد نبود!';
    }
  ...
  showReplay(replayText) {
    ...
    this.stage.hud.replayButton = replayText + ' دوباره بازی کنید؟';
  }
